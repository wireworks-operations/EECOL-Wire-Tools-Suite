...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // MD3 UI Control Synchronizations
    syncMD3ChipState("btnBatchEntryMode", "batchEntryMode");
    syncMD3ChipState("btnSingleUnitCut", "singleUnitCut");
    syncMD3ChipState("btnFullPick", "fullPick");
    syncMD3ChipState("btnNoMarks", "noMarks");
    syncMD3ChipState("btnSystemCut", "systemCut");
    syncMD3ChipState("btnCutInSystem", "cutInSystem");
    syncMD3ChipState("btnReReel", "reReel");

    syncSegmentedGroup("segGroupLengthUnit", "cutLengthUnit");
    syncSegmentedGroup("segGroupCoilOrReel", "coilOrReel");

    const toggleOptional = document.getElementById("toggleOptionalDetails");
    const optionalContent = document.getElementById("optionalDetailsContent");
    const optionalChevron = document.getElementById("optionalDetailsChevron");
    if (toggleOptional && optionalContent) {
        toggleOptional.addEventListener("click", () => {
            ...
            if (isHidden) {
                ...
                if (optionalChevron) optionalChevron.textContent = "▼";
            } else {
    ...
    async function checkActiveOrderState() {
        if (window.eecolDB && await window.eecolDB.isReady()) {
            ...
            const activeItem = list.find(i => (i.status === 'active' || !i.status) && (i.isActive || i.isGroupActive));
...
function renderWireCutList() {
    ...
    filtered.forEach(item => {
        ...
        const bgColor = item.color || '#fef08a';
...
async function deleteWireListItem(id) {
    try {
        if (window.eecolDB && await window.eecolDB.isReady()) {
            ...
            await loadWireCutList();
            if (typeof checkActiveOrderState === 'function') {
                await checkActiveOrderState();
            }
        }
    } catch (error) {
...
// MD3 UI Helper Functions
function syncMD3ChipState(chipBtnId, checkboxId) {
    const chipBtn = document.getElementById(chipBtnId);
    const checkbox = document.getElementById(checkboxId);
    if (!chipBtn || !checkbox) return;
